...
describe('Release 1.1.9 changes ...', function () {
    ...
    //     cy.visit('').create_new_user(userData[0])
    ...
    it('...updating a listing data.', function () {
        ...
        cy.get('#listing_category_id').select(userData[0]['category']).should('have.value', '4')
        cy.wait(1000) // wait for ajax response
        ...
        cy.get('#listing_subcategory_id').select(userData[0]['sub_category']).contains('Activities & Events')
    })
